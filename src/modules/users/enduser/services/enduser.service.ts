...
export class EndUserService implements IEndUserService {
  ...
  public async addStar(userId: EndUserId, star: number) {
    const user = await this.endUserRepository.findById(userId);
    console.log(star, user.star + star, typeof (user.star + star));
    user.star = user.star + star;
    console.log(user.star, user);
    return user.save();
  }
}
