...
export class PostService implements IPostService {
  ...
  public async getRecommendedPosts<T>({
    ...
    const queryAggregation: PipelineStage[] = [
      {
        // this will match posts that are not from a group
        $match: {
          groupId: { $exists: false },
        },
      },
      ...
      },
      {
        $unwind: {
          path: '$userFull',
          preserveNullAndEmptyArrays: true, // This allows records without userFull to stay in the result
        },
      },
      {
        $set: {
  ...
  public async modifyPost({
    ...
    Object.assign(post, {
      ...modifyPostDto,
      images: images.length == 0 ? post.images : images,
    });
    await post.save();
    return post;
  }
